...
class omar_sevice1Service:
    ...
    def find_by_id(self, entity_id):
        ...
        data = omar_sevice1.query.get(entity_id)
        if data is None:
            raise omar_sevice1ServiceError(
                'omar_sevice1 {} not found'.format(entity_id),
    ...
    def get_all(self):
        list = []
        data = omar_sevice1.query.filter(
            omar_sevice1.tenantId == self.auth_context.tenant_id)
    ...
    def update(self, model, entity_id):
        ...
        record = omar_sevice1.query.get(entity_id)  # omar_sevice1 is a mongo class
        if record is None:
            raise omar_sevice1ServiceError(
                'omar_sevice1 {} not found'.format(entity_id),
                ErrorCodes.NOT_FOUND)
